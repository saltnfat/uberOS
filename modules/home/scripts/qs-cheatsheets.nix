...
pkgs.writeShellScriptBin "qs-cheatsheets" ''
  ...
  EOF
    ...
    if [[ "$CATEGORY" != "root" && ! -d "$HOME/uberOS/cheatsheets/$CATEGORY" ]]; then
      echo "Error: Category directory '$CATEGORY' not found in cheatsheets/" >&2
      echo "Available categories:" >&2
      echo "  root" >&2
      ls -1 "$HOME/uberOS/cheatsheets/" | grep -E '^[a-z]' | head -10 >&2
      exit 1
    fi
    ...
    if [ -d "$HOME/uberOS/cheatsheets" ]; then
      for category_dir in "$HOME/uberOS/cheatsheets"/*/; do
  ...
  Window {
    ...
    function loadFileContent(filename) {
      ...
      if (selectedCategory === "root") {
        filePath = "${config.home.homeDirectory}/uberOS/cheatsheets/" + filename;
      } else {
        filePath = "${config.home.homeDirectory}/uberOS/cheatsheets/" + selectedCategory + "/" + filename;
      }

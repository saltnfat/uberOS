...
pkgs.writeShellScriptBin "keybinds-parser" ''
    ...
    case "$MODE" in
      hyprland)
        # First, try to parse uberOS bindd entries from Nix config for richer descriptions
        BIND_NIX="$HOME/uberOS/modules/home/hyprland/binds.nix"
  ...
  BEGIN {
      ...
      sway)
        ...
        if [[ ! -f "$SWAY_CONFIG" ]]; then
          SWAY_CONFIG="$HOME/uberOS/modules/home/gui/sway/files/config_cuerdos"
          if [[ ! -f "$SWAY_CONFIG" ]]; then
